...
exports.findDevice = function(vid, pid) {
    return new Promise(function(fulfill, reject) {
        if (!vid) {
            reject(new Error('Vendor ID must be non-empty'));
        }

        if (!pid) {
            reject(new Error('Product ID must be non-empty'));
        }
...
_.extend(WebUSBDevice.prototype, {
    ...
    transfer: function(buffer) {
        return new Promise(_.bind(function(fulfill, reject) {
            if (!this._outEndpoint) {
                reject(new Error('Claim the interface before start ransfer'));
            }

            var tranfer = this._outEndpoint.transfer(buffer, function(err) {
                if (err) {
                    reject(new Error('USB transfer failed:' + err));
                } else {
                    fulfill();
                }
            });

            if ((typeof tranfer === 'undefined')) {
                reject(new Error('USB transfer failed'));
            }
        }, this));
    },
    ...
    reset: function() {
        return new Promise(_.bind(function(fulfill, reject) {
            this._device.reset(function(err) {
                if (err) {
                    reject(new Error('Failed to reset the device - ' + err));
                 } else {
